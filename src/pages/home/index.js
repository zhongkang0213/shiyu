import React, { Component } from 'react';
import { observer, inject } from "mobx-react";
import './index.scss';

// import update from 'immutability-helper'
import BasicWrapper from 'components/BasicWrapper'
import DropList from './children/DropList'
import DropArea from './children/DropArea'
import DropView from './children/DropView'
import { message } from 'antd';
message.config({
    maxCount: 1,
    top: 200
});
@inject('homeStore')
@observer

class Home extends Component {
  state = {
    data: [
      {
        title: '炒菜（素）',
        list: [
          {
            caiyao: {
              name: '清炒包菜',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            },
            shicai: {
              name: '圆白菜、卷心菜',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            }
          },
          {
            caiyao: {
              name: '清炒香芹',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            },
            shicai: {
              name: '香芹',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            }
          },
          {
            caiyao: {
              name: '番茄鸡蛋',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            },
            shicai: {
              name: '番茄、鸡蛋',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            }
          },
          {
            caiyao: {
              name: '清炒油麦菜',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            },
            shicai: {
              name: '油麦菜',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            }
          }
        ]
      }, {
        title: '炒菜（荤）',
        list: [
          {
            caiyao: {
              name: '红烧茄子',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            },
            shicai: {
              name: '茄子、土豆',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            }
          },
          {
            caiyao: {
              name: '青椒杏鲍菇炒牛柳',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            },
            shicai: {
              name: '青椒、杏鲍菇、牛肉',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            }
          },
          {
            caiyao: {
              name: '木须肉',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            },
            shicai: {
              name: '黄瓜、鸡蛋、木耳',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            }
          },
          {
            caiyao: {
              name: '红烧鱼块',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            },
            shicai: {
              name: '鱼',
              count: 30,
              nengliang: 3.7,
              nengliang_s: 0.3,
              danbaizhi: 1.2,
              danbaizhi_s: 0.13
            }
          }
          // {
          //   name: '红烧茄子'
          // }, {
          //   name: '青椒杏鲍菇炒牛柳'
          // }, {
          //   name: '木须肉'
          // }, {
          //   name: '红烧鱼块'
          // }
        ]
      }
    ],
    tableData: new Array(5).fill({morning: [], aftern: [], evening: []}),
    // tableData: {
    //   morning: [
    //     // {
    //     //   caiyao: {
    //     //     name: '清炒包菜',
    //     //     count: 30
    //     //   },
    //     //   shicai: {
    //     //     name: '圆白菜、卷心菜',
    //     //     count: 30
    //     //   }
    //     // }
    //   ],
    //   aftern: [
        
    //   ],
    //   evening: [
    //     // {
    //     //   caiyao: {
    //     //     name: '清炒包菜',
    //     //     count: 30
    //     //   },
    //     //   shicai: {
    //     //     name: '圆白菜、卷心菜',
    //     //     count: 30
    //     //   }
    //     // },
    //     // {
    //     //   caiyao: {
    //     //     name: '清炒包菜',
    //     //     count: 30
    //     //   },
    //     //   shicai: {
    //     //     name: '圆白菜、卷心菜',
    //     //     count: 30
    //     //   }
    //     // }
    //   ]
    // },
    viewTableData: [
      {
        key: 'nengliang',
        name: '能量',
        biaozhun: 0,
        sheru: 0,
        baifenbi: '0.0%'
      },
      {
          key: 'danbaizhi',
          name: '蛋白质',
          biaozhun: 0,
          sheru: 0,
          baifenbi: '0.0%'
      }
    ],
    tableTabIndex: 1
  }

  componentDidMount = () => {
      this.props.homeStore.initPage()
  }

  setViewTableData = () => {
    let tableData = JSON.parse(JSON.stringify(this.state.tableData))
    let viewTableData = JSON.parse(JSON.stringify(this.state.viewTableData))
    let { morning, aftern, evening } = tableData
    let data = [].concat(morning, aftern, evening)
    viewTableData[0].biaozhun = 0
    viewTableData[0].sheru = 0
    viewTableData[0].baifenbi = '2.2%'
    viewTableData[1].biaozhun = 0
    viewTableData[1].sheru = 0
    viewTableData[1].baifenbi = '0.95%'
    data.map((item) => {
      viewTableData[0].biaozhun = (+viewTableData[0].biaozhun + item.caiyao.nengliang * item.caiyao.count  + item.shicai.nengliang * item.shicai.count).toFixed(2)
      viewTableData[0].sheru = (+viewTableData[0].sheru + item.caiyao.nengliang_s * item.caiyao.count  + item.shicai.nengliang_s * item.shicai.count).toFixed(2)
      viewTableData[1].biaozhun = (+viewTableData[1].biaozhun + item.caiyao.danbaizhi * item.caiyao.count  + item.shicai.danbaizhi * item.shicai.count).toFixed(2)
      viewTableData[1].sheru = (+viewTableData[1].sheru + item.caiyao.danbaizhi_s * item.caiyao.count  + item.shicai.danbaizhi_s * item.shicai.count).toFixed(2)
      return null
    })
    this.setState({
      viewTableData
    })
    // console.log(viewTableData)
  }

  handleDropArea = (data) => {
    let tableData = JSON.parse(JSON.stringify(this.state.tableData))
    let { tableTabIndex } = this.state
    tableData[tableTabIndex].evening.push(data)
    this.setState({
      tableData
    }, () => {
    //   this.setViewTableData()
    })
  }

  handleTableItem = (data, key) => {
    // console.log('handleTableItem')
    let tableData = JSON.parse(JSON.stringify(this.state.tableData))
    if (!this._checkTableId(data, key)) {
        message.warning('请勿重复添加相同菜肴')
        return
    }
    let { tableTabIndex } = this.state
    data.count = null
    tableData[tableTabIndex][key].push(data)
    this.setState({
      tableData
    }, () => {
    //   this.setViewTableData()
    })
  }

  closeCyTag = (key, index) => {
    let tableData = JSON.parse(JSON.stringify(this.state.tableData))
    let { tableTabIndex } = this.state
    tableData[tableTabIndex][key].splice(index, 1)
    this.setState({
      tableData
    }, () => {
    //   this.setViewTableData()
    })
  }

  onNumberChange = (key, type, index, value) => {
    let tableData = JSON.parse(JSON.stringify(this.state.tableData))
    let { tableTabIndex } = this.state
    tableData[tableTabIndex][key][index].count = value
    this.setState({
      tableData
    }, () => {
    //   this.setViewTableData()
    })
  }

  handleTabsChange = (key) => {
    this.setState({
        tableTabIndex: key
    })
  }

  _checkTableId = (data, key) => {
    let tableData = JSON.parse(JSON.stringify(this.state.tableData))
    let { tableTabIndex } = this.state
    let checkData = tableData[tableTabIndex][key]
    for (let i = 0; i < checkData.length; i++) {
        if (+data.id === +checkData[i].id) {
            return false
        }
    }
    return true
  }

  render() {
    let { data, tableData, viewTableData, tableTabIndex } = this.state
    let { foodsList } = this.props.homeStore
    return (
        <BasicWrapper width={1200} height={550}>
            <div className="home-wrapper">
                <div>
                    <DropList data={foodsList} />
                </div>
                <div>
                    <DropArea handleDropArea={this.handleDropArea} 
                        onNumberChange={this.onNumberChange} 
                        closeCyTag={this.closeCyTag} 
                        tableData={tableData[tableTabIndex]} 
                        handleTableItem={this.handleTableItem}
                        handleTabsChange={this.handleTabsChange}
                        defaultTableIndex={''+tableTabIndex} />
                </div>
                {/* <div>
                    <DropView data={viewTableData} />
                </div> */}
            </div>
      </BasicWrapper>
    );
  }
}

export default Home;
